...
function play(sound) {
	...
	const prefix = 
	"http://ctp.di.fct.unl.pt/miei/lap/projs/proj2018-3/files/resources/sounds/";
	if( soundEnabled )
...
function div(target, style, typeOf) {
	...
	a.style.boxShadow = "0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 "+
	"rgba(0,0,0,0.19)";
	return a;    
}
...
static onMouseOverFireBrick (event) {
	...
	event.target.style.boxShadow = " 0 2px 8px 0 rgba(0,0,0,0.2), 0 3px 10px 0 "+
	"rgba(0,0,0,0.19)";
}
...
saveSoundPrefix(xmlDocument) {
	this.soundPrefix = 
	xmlDocument.getElementsByTagName("SOUNDSPREFIX")[0].childNodes[0].nodeValue;
}
...
pageRendering() {
	...
	for (var i = 0; i < this.numLessons;i++) {
		...
		buttons[i] = 
		inpuButton(p2, "Check", "Lesson: "+(i+1), "Khaki", "FireBrick", null, 3);
		var h0 = 
		h1(d[i], "Completed Exercises: "+concludedExercises+"/"+nrExercises, 4);
	...
	for(var w = 0; w < this.numSymbolsLessons; w++){
		...
		symButtons[w] = 
		inpuButton(p3, "Check", "Symbols: "+(w+1), "white", "FireBrick", null, 3);
		const indexw = w;
...
pageRendering() {
	...
	if(this.lesson.getConcludedExercises() == this.lesson.getNumberExercises()){
		...
		var b = 
		inpuButton(p1, "check", "No! Go to Home page", "white", null, null, 4);
...
keyboardPrompt() {
	this.prompt = 
	this.xmlKeyInfo.getElementsByTagName("PROMPT")[0].childNodes[0].nodeValue;
}

keyboardOriginal() {
	this.original = 
	this.xmlKeyInfo.getElementsByTagName("ORIGINAL")[0].childNodes[0].nodeValue;
}

keyboardSound() {
	if(this.xmlKeyInfo.getElementsByTagName("SOUND").length != 0){
		this.sound = 
		this.xmlKeyInfo.getElementsByTagName("SOUND")[0].childNodes[0].nodeValue;
	}
...
pageRendering() {
	...
	eventHandler2(b1, "onclick", function() {
		concludedExercise = 
		validate(document.getElementById('answer').value, self.translations);
...
pairsPrompt() {
	this.prompt = 
	this.xmlPairs.getElementsByTagName("PROMPT")[0].childNodes[0].nodeValue;
}

pairsOriginal() {
	this.original = 
	this.xmlPairs.getElementsByTagName("ORIGINAL")[0].childNodes[0].nodeValue;
}

pairsSolution() {
	this.solution = 
	this.xmlPairs.getElementsByTagName("SOLUTION")[0].childNodes[0].nodeValue;
}
...
blocksPrompt() {
	this.prompt = 
	this.xmlBlocks.getElementsByTagName("PROMPT")[0].childNodes[0].nodeValue;
}

blocksOriginal() {
	this.original = 
	this.xmlBlocks.getElementsByTagName("ORIGINAL")[0].childNodes[0].nodeValue;
}

blocksBlocks() {
	this.block = 
	this.xmlBlocks.getElementsByTagName("BLOCKS")[0].childNodes[0].nodeValue;
}

blocksSolution() {
	this.solution = 
	this.xmlBlocks.getElementsByTagName("SOLUTION")[0].childNodes[0].nodeValue;
}
...
symbolsPrompt(xmlSymbols) {
	...
	if (xmlSymbols.getElementsByTagName("PROMPT").length > 0){
		this.prompt = 
		xmlSymbols.getElementsByTagName("PROMPT")[0].childNodes[0].nodeValue;
	}
}
...
symbolsSoundDir(xmlSymbols) {
	...
	if (xmlSymbols.getElementsByTagName("SOUNDSDIR").length > 0){
		this.soundDir = 
		xmlSymbols.getElementsByTagName("SOUNDSDIR")[0].childNodes[0].nodeValue;
	}
}
...
symbolsComment(xmlSymbols) {
	...
	if (xmlSymbols.getElementsByTagName("COMMENT").length > 0){
		this.commentToShow = 
		xmlSymbols.getElementsByTagName("COMMENT")[0].childNodes[0].nodeValue;
	}
}
